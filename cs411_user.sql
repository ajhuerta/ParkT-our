...
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `user_BEFORE_INSERT` BEFORE INSERT ON `user` FOR EACH ROW BEGIN
    ...
    -- Concatenate numbers(1, 2, ...) after used UserID
    WHILE @x IS NOT NULL DO
        SET @tmp2 = NEW.UserID;
        SET @y = CONCAT(@tmp2, @tmp);
	...
	-- set NEW.UserID = concatenated UserID
    SET NEW.UserID = @y;
END */;;
DELIMITER ;
...
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `user_AFTER_UPDATE` AFTER UPDATE ON `user` FOR EACH ROW BEGIN
	SET @email = (SELECT Plan_Confirmed.Email
					FROM Plan_Confirmed
                    WHERE Plan_Confirmed.Email = OLD.Email
                    LIMIT 1);
	IF @email IS NOT NULL THEN
		UPDATE Plan_Confirmed
		SET Plan_Confirmed.Email = NEW.Email
        WHERE Plan_Confirmed.Email = OLD.Email;
    END IF;
END */;;
DELIMITER ;
